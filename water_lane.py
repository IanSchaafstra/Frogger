...
class WaterLane:
    ...
    def get_log_at_position(
        self, player_rect
    ):  # Check if player is on a log with sufficient overlap.
        MIN_OVERLAP = 25  # Require 20px overlap to count as "on log"

        for log in self.logs:
            if player_rect.colliderect(log.rect):
                # Calculate overlap dimensions
                overlap_width = min(player_rect.right, log.rect.right) - max(
                    player_rect.left, log.rect.left
                )
                overlap_height = min(player_rect.bottom, log.rect.bottom) - max(
                    player_rect.top, log.rect.top
                )
